      ...
      <div className="auth-root">
        ...
        <div className="auth-right">
          <div className="auth-form-wrap">
            ...
            <h1 className="auth-heading">Sign in to<br /><em>MoodMarket</em></h1>
            <p className="auth-subheading">Your mood-curated cart is waiting for you.</p>
            ...
            <div className="auth-field">
              <div className="auth-field-header">
                ...
                <button className="auth-field-link" onClick={() => { setResetEmail(email); setForgotOpen(true); setResetSent(false); }}>
                  Forgot password?
                </button>
              </div>
            ...
            <div className="auth-modal" style={{ position: 'relative' }} onClick={e => e.stopPropagation()}>
              ...
              {resetSent ? (
                <>
                  ...
                  <button className="auth-modal-btn" onClick={() => setForgotOpen(false)}>Done</button>
                </>
              ) : (
                <>
                  <div className="auth-modal-title">Reset Password</div>
                  <div className="auth-modal-sub">Enter your email and we'll send you a link to reset your password.</div>
                  <input
                    ...
                    autoFocus
                  />
                  <button className="auth-modal-btn" onClick={handleReset}>Send Reset Link</button>
                  <button className="auth-modal-cancel" onClick={() => setForgotOpen(false)}>Cancel</button>
                </>
              )}
            </div>
          </div>
...
function LoginScreenMobile() {
  ...
  const handleForgotPassword = () => { setResetEmail(email); setForgotVisible(true); };

  const getRedirectURL = () => {
    if (Platform.OS === 'web') return `${window.location.origin}/reset-password`;
  ...
  const handleForgotSubmit = async () => {
    ...
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(resetEmail.trim(), { redirectTo: getRedirectURL() });
      if (error) throw error;
      Alert.alert('Check your email', 'We sent a reset link to your inbox.');
      setForgotVisible(false); setResetEmail('');
    } catch (err: any) {
      ...
      <ScrollView contentContainerStyle={ms.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={ms.card}>
          ...
          <View style={[ms.inputBox, passwordFocused && ms.inputFocused]}>
            ...
            />
            <TouchableOpacity onPress={() => setShowPassword(!showPassword)} style={ms.eyeBtn}>
              {showPassword
      ...
      {forgotVisible && (
        <View style={ms.modalOverlay}>
          <View style={ms.modalBox}>
            ...
            <Text style={ms.modalSub}>Enter your email and we'll send you a reset link.</Text>
            <TextInput
              value={resetEmail}
              ...
              style={ms.modalInput}
            />
            <TouchableOpacity style={ms.modalButton} onPress={handleForgotSubmit} activeOpacity={0.85}>
